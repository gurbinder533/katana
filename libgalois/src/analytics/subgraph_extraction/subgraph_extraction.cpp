...
SubGraphNodeSet(
  ...
  katana::GAccumulator<uint64_t> accum_num_edges;
  katana::do_all(
      katana::iterate(node_set),
      [&](const uint32_t& n) {
        for (auto dest : node_set) {
          auto edge_range = graph->topology().edge_range(n);
          auto edge_matched = std::lower_bound(
              edge_iterator(edge_range.first), edge_iterator(edge_range.second),
              dest, [=](edge_iterator e, uint32_t n) {
                return graph->topology().edge_dest(*e) < n;
              });
  ...
  uint64_t num_edges = accum_num_edges.reduce();
...
katana::analytics::SubGraphExtraction(
    katana::PropertyGraph* pg, const std::vector<uint32_t>& node_set,
  ...
  case SubGraphExtractionPlan::kNodeSet:
    auto subgraph = SubGraphNodeSet(pg, node_set);
