...
main(int argc, char** argv) {
  ...
  if (!nodesFile.getValue().empty()) {
    ...
    node_set.insert(
        node_set.end(), std::istream_iterator<uint64_t>{file},
        std::istream_iterator<uint64_t>{});
  } else {
    std::istringstream str(nodesString);
    node_set.insert(
        node_set.end(), std::istream_iterator<uint64_t>{str},
        std::istream_iterator<uint64_t>{});
  }
  ...
  auto subgraph_result = SubGraphExtraction(pg.get(), node_set, plan);
  if (!subgraph_result) {
